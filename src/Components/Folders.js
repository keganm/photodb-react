...
export const Folders = (props) => {
  ...
  function GetFolders() {
    ...
    GoogleDrive.GetFolderList(OnFolderListReturned, {
      query:
        QUERIES.FoldersOnly() + " and " + QUERIES.ParentIs(props.currentFolder),
    });
  }

  useEffect(() => {
    if (!props.gapiInterface) return;
  ...
  const FolderGridArray =
    folders != null
      ...
      : null;
  return (
    <> {ShowFolders() && <Card title="Folders"> {FolderGridArray} </Card>} </>
  );
};
